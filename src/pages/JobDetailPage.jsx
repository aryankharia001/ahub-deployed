...
const JobDetailPage = () => {
  ...
  const handleApproveWork = async (feedback) => {
    ...
    try {
      ...
      }, 1000); // 1 second delay so the user can see the notification
  ...
  const renderPaymentButton = () => {
    ...
    } else if (job.status === 'approved_by_client') {
      return (
        <button 
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-between">
                  {['pending', 'approved', 'deposit_paid', 'in_progress', 'completed', 'final_paid'].map((status, index) => {
                    const isActive = 
                      ['pending', 'approved', 'deposit_paid', 'in_progress', 'completed', 'approved_by_client', 'final_paid', 'job_end'].indexOf(job.status) >= 
                      ['pending', 'approved', 'deposit_paid', 'in_progress', 'completed', 'approved_by_client', 'final_paid', 'job_end'].indexOf(status);
  ...
  <div className="relative">
    ...
    <div className="relative flex justify-between">
      {['pending', 'approved', 'deposit_paid', 'in_progress', 'completed', 'final_paid'].map((status, index) => {
        ...
        const isActive = 
          ['pending', 'approved', 'deposit_paid', 'in_progress', 'completed', 'approved_by_client', 'final_paid', 'job_end'].indexOf(normalizedStatus) >=
          ['pending', 'approved', 'deposit_paid', 'in_progress', 'completed', 'approved_by_client', 'final_paid', 'job_end'].indexOf(status);

        const isCurrent = normalizedStatus === status;
        ...
        return (
          <div key={status} className="flex flex-col items-center">
            ...
            {job.client && (
              <div className="mb-6">
                ...
                <span className="text-blue-600">
                  {job?.client?.name || (job?.client?._id ? `Client #${job?.client?._id.substring(0, 8)}` : 'Client')}
                </span>
                </div>
              </div>
            )}
            ...
            {/* Client Review Buttons - Add within deliverables section */}
            {job.client._id === user._id && 
              (job.status === 'final_paid' || job.status === 'revision_completed') && (
              <div className="mt-4 bg-indigo-50 p-4 rounded-lg">
                <h4 className="text-md font-medium text-indigo-800 mb-2">Review Submission</h4>
                <p className="text-sm text-indigo-700 mb-3">
                  Please provide your feedback on the submitted work:
                </p>
                <div className="flex space-x-3">
                  <button
                    onClick={() => setShowApprovalModal(true)}
                  ...
                  {job.revisionsRemaining > 0 && (
                    <button
                      onClick={() => setShowRevisionModal(true)}
                      className="inline-flex items-center px-4 py-2 bg-white text-orange-700 border border-orange-300 rounded-md shadow-sm hover:bg-orange-50"
                    >
                      <RotateCcw className="mr-1 h-4 w-4" />
                      Request Revision ({job.revisionsRemaining} left)
                    </button>
                  )}
                </div>
              </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-medium text-blue-800">
                        {user.role === 'client' && (job.status === 'completed' || job.status === 'revision_completed' || job.status === 'approved_by_client')
                    ...
                    {user.role === 'client' && (job.status === 'completed' || job.status === 'revision_completed') && (
                      <div className="mt-4">
                        <div className="bg-white p-3 rounded-lg border border-orange-200">
                          <p className="text-sm text-gray-700 mb-3">
                            The contributor has completed this job and uploaded files for you. Please review the work first. After approval, you can make the final payment to unlock the files.
                          </p>
                        </div>
                      </div>
                    )}
                    
                    {user.role === 'client' && job.status === 'approved_by_client' && (
                      <div className="mt-4">
                        <div className="bg-white p-3 rounded-lg border border-orange-200">
                          <p className="text-sm text-gray-700 mb-3">
                            You have approved the work. Please complete the final payment to unlock the full version of the files.
                          </p>
