...
const ContributorDashboardPage = () => {
  ...
  const handleSubmitRevision = async (revisionId, job) => {
  ...
  // Updated handleRevisionSubmission function for the ContributorDashboardPage
...
//     const uploadSource = formData.get('uploadSource');
//     console.log('Submitting revision with data:', {
//       jobId: jobDetails?._id,
//       revisionId: selectedRevision,
//       uploadSource: uploadSource,
//       filesCount: uploadSource === 'local' ? formData.getAll('files').length : 'Using Google Drive files'
//     });
...
//     // Update the current job details to show the updated revision
//     const updatedJob = jobsResponse.data.data.find(job => job._id === jobDetails?._id);
//     if (updatedJob) {
...
//     }
//     setError(err.response?.data?.message || 'Failed to submit revision. Please try again.');
//   } finally {
//     setIsSubmitting(false);
//   }
// };

// Updated handleRevisionSubmission function for ContributorDashboardPage
// const handleRevisionSubmission = async (formData) => {
//   setIsSubmitting(true);
//   setUploadProgress(0);
...
//     // Log form data for debugging
//     const uploadSource = formData.get('uploadSource');
//     const hasRevisionId = formData.has('revisionId');
    
//     console.log('Submitting revision with data:', {
//       jobId: jobDetails?._id,
//       revisionId: selectedRevision,
//       uploadSource: uploadSource,
//       hasRevisionId: hasRevisionId,
//       messageContent: formData.get('message')
//     });
    
//     if (uploadSource === 'drive') {
//       const driveFilesStr = formData.get('driveFiles');
//       if (driveFilesStr) {
//         console.log(`Selected ${JSON.parse(driveFilesStr).length} Google Drive files`);
//       }
//     } else {
//       console.log(`Selected ${formData.getAll('files').length} local files`);
//     }
                  ...
                  {job.status === 'revision_in_progress' && (
                      <button
                        onClick={() => {
                          ...
                          if (activeRevision) {
                            handleSubmitRevision(activeRevision._id, job);
                          }
                        }}
  ...
  <div className="fixed inset-0 z-10 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
      ...
      <form onSubmit={(e) => {
        ...
        // Validation checks
        if (!jobDetails?._id) {
          setSubmitError("Job details not available. Please refresh the page and try again.");
          return;
        }
        ...
        <div className="mt-5 sm:mt-6 flex justify-end space-x-3">
          ...
          <button
            type="submit"
            disabled={isSubmitting || selectedFiles.length === 0 || !jobDetails?._id}
            className={`inline-flex justify-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white ${
              isSubmitting || selectedFiles.length === 0 || !jobDetails?._id
...
{/* Revision Submit Modal */}
{showRevisionSubmitModal && (
  <div className="fixed inset-0 z-10 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
      <div className="flex justify-between items-start mb-4">
        <h3 className="text-lg font-medium text-gray-900">Submit Revision for "{jobDetails?.title}"</h3>
        <button
          onClick={() => setShowRevisionSubmitModal(false)}
          className="text-gray-400 hover:text-gray-500"
        >
          <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
      ...
      <form onSubmit={(e) => {
        ...
        if (!selectedRevision) {
          setSubmitError("No revision selected. Please select a revision to submit work for.");
          return;
        }
        ...
        // Add the revision ID
        formData.append("revisionId", selectedRevision);
        
        // Append each file individually with the 'files' field name
        selectedFiles.forEach(file => {
          console.log(`Uploading revision file: ${file.name}, type: ${file.type}, size: ${file.size} bytes`);
          formData.append("files", file);
        });
        ...
        console.log(`Submitting revision with ${selectedFiles.length} files for job: ${jobDetails._id}, revisionId: ${selectedRevision}`);
        
        // Submit the form data
        axios.post(`${Backendurl}/api/jobs/${jobDetails._id}/submit-work`, formData, {
          headers: {
            // Don't set Content-Type here - let Axios set it automatically with boundary
            Authorization: `Bearer ${localStorage.getItem('token')}`
          },
          onUploadProgress: (progressEvent) => {
            const percentCompleted = Math.round((progressEvent.loaded * 100) / progressEvent.total);
            setUploadProgress(percentCompleted);
            console.log(`Revision upload progress: ${percentCompleted}%`);
          }
        })
        .then(response => {
          console.log('Revision submission successful:', response.data);
          setNotification('Revision submitted successfully! The client will be notified to review it.');
          
          // Update the job in myJobs list
          setMyJobs(prevJobs => 
            prevJobs.map(job => 
              job._id === jobDetails._id ? { ...job, status: 'revision_completed' } : job
            )
          );
        ...
        .catch(err => {
          ...
          if (err.response) {
            console.error('Server error details:', err.response.data);
            setSubmitError(err.response.data.message || 'Failed to submit revision. Please try again.');
          } else if (err.request) {
            setSubmitError('Network error. Please check your connection and try again.');
          } else {
            setSubmitError('An unexpected error occurred. Please try again.');
        ...
        <div className="mb-4">
          ...
          <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
            <div className="space-y-1 text-center">
              ...
              <div className="flex text-sm text-gray-600">
                <label htmlFor="revision-file-upload" className="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                  <span>Upload files</span>
                  <input 
                    ...
                    onChange={(e) => {
                      if (e.target.files && e.target.files.length > 0) {
                        const files = Array.from(e.target.files);
                        console.log(`Selected ${files.length} revision files`);
                        setSelectedFiles(files);
                      }
                    }}
                    ref={fileInputRef}
        ...
        <div className="mb-4">
          <label htmlFor="message" className="block text-sm font-medium text-gray-700 mb-2">
            Message to Client (Explain your revisions)
          </label>
          <textarea
            id="message"
            name="message"
            rows="3"
            className="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md"
            placeholder="Explain the changes you've made based on client feedback..."
          ></textarea>
        </div>
        ...
        <div className="mt-5 sm:mt-6 flex justify-end space-x-3">
          ...
          <button
            type="submit"
            disabled={isSubmitting || selectedFiles.length === 0 || !jobDetails?._id || !selectedRevision}
            className={`inline-flex justify-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white ${
              isSubmitting || selectedFiles.length === 0 || !jobDetails?._id || !selectedRevision
                ? 'bg-indigo-400 cursor-not-allowed'
                : 'bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'
            }`}
          >
